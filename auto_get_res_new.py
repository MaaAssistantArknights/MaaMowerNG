...
class Arknights数据处理器:
            ...
            for i in os.listdir(data_path):
                ...
                for j, k in mapping.items():
                    ...
                    if i.startswith(j):
                        ...
                        kp, des = keypoints(img)
                        if k not in data:
                            data[k] = []
                        data[k].append(des)
                        break
            with lzma.open(f"./arknights_mower/models/{name}.pkl", "wb") as f:
                pickle.dump(data, f)

        extract("avatar", "WHITE", ((47, 46), (147, 146)))
        extract("portrait", (140, 140, 140, 255), ((40, 60), (140, 160)))
    ...
    def levels(self):
        levels_json = self.加载json("./ArknightsGameResource/levels.json")
        with lzma.open("arknights_mower/models/levels.pkl", "wb") as f:
