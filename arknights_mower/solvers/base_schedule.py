...
from arknights_mower.solvers.get_agent_from_room import GetAgentFromRoomSolver
...
class BaseSchedulerSolver(SceneGraphSolver, BaseMixin):
    ...
    def infra_main(self):
        ...
        if self.task is not None:
            ...
            if self.no_pending_task(1):
                notification = detector.infra_notification(config.recog.img)
        ...
        try:
            ...
            while tasks:
                ...
                if scene == Scene.UNKNOWN:
                    ...
                    self.tap(
                        (config.recog.w * 0.05, config.recog.h * 0.95), interval=0.5
                    )
                    ...
                    if tasks[0] == "upgrade":
                        # 进入技能选择界面
                        self.tap(
                            (config.recog.w * 0.05, config.recog.h * 0.95),
                            interval=0.5,
                        )
                    if tasks[0] == "confirm":
                        # 读取专精倒计时 如果没有，判定专精失败
                    ...
                    if tasks[0] == "upgrade":
                        ...
                        self.ctap((config.recog.w * 0.33, config.recog.h * height))
                    else:
                        self.back()
                    ...
                    if tasks[0] == "upgrade":
                        ...
                        # 点击确认开始专精
                        self.tap((config.recog.w * 0.87, config.recog.h * 0.9))
                        del tasks[0]
                    else:
                        self.back()
        ...
        while (
            ...
            if self.scene() in self.waiting_scene:
                if not self.waiting_solver():
                    return
            self.tap((config.recog.w * 1320 // 1920, config.recog.h * 502 // 1080))
            if self.scene() in self.waiting_scene:
                if not self.waiting_solver():
                    return
            self.tap((config.recog.w * 3 // 4, config.recog.h * 4 // 5))
            if self.scene() in self.waiting_scene:
                if not self.waiting_solver():
                    return
            while self.find("bill_accelerate") is None:
                ...
                self.tap((config.recog.w // 20, config.recog.h * 19 // 20), interval=1)
                error_count += 1
            _time = self.double_read_time(
                (
                    (config.recog.w * 650 // 2496, config.recog.h * 660 // 1404),
                    (config.recog.w * 815 // 2496, config.recog.h * 710 // 1404),
                ),
                use_digit_reader=True,
            )
    ...
    def choose_train_agent(
        ...
        if current_room[idx] != agents[idx]:
            while (
                ...
                self.ctap((config.recog.w * 0.82, config.recog.h * 0.18 * (idx + 1)))
                error_count += 1
    ...
    def choose_agent(
        ...
        if fast_mode:
            ...
            for pos in differences:
                if current_room[pos] != "":
                    self.tap(
                        (
                            config.recog.w * position[pos][0],
                            config.recog.h * position[pos][1],
                        ),
                        interval=0,
                    )
        ...
        while len(agent) > 0:
            ...
            if first_time:
                ...
                if not fast_mode:
                    self.tap(
                        (config.recog.w * 0.38, config.recog.h * 0.95), interval=0.5
                    )
        ...
        if free_num:
            if free_num == len(agents):
                self.tap((config.recog.w * 0.38, config.recog.h * 0.95), interval=0.5)
            if not first_time:
                # 滑动到最左边
                self.sleep(interval=0.5)
                right_swipe = self.swipe_left(right_swipe)
        ...
        if len(agents) != 1:
            ...
            for idx, item in enumerate(agents):
                if agents[idx] != exists[idx] or not_match:
                    ...
                    self.tap(
                        (
                            config.recog.w * position[p_idx][0],
                            config.recog.h * position[p_idx][1],
                        ),
                        interval=0,
                    )
                    self.tap(
        ...
        if len(new_plan) == 1:
            if config.conf.run_order_buffer_time <= 0:
                ...
                execute_time = self.double_read_time(
                    (
                        (
                            int(config.recog.w * 650 / 2496),
                            int(config.recog.h * 660 / 1404),
                        ),
                        (
                            int(config.recog.w * 815 / 2496),
                            int(config.recog.h * 710 / 1404),
                        ),
                    ),
                    use_digit_reader=True,
                )
                ...
                while (
                    ...
                    self.tap(
                        (config.recog.w * 0.25, config.recog.h * 0.25), interval=0.5
                    )
    ...
    def initialize_maa(self):
        ...
        if self.MAA.connect(
            conf.maa_adb_path, config.device.client.device_id, conf.maa_conn_preset
        ):
            ...
            if (conf.RG or conf.SSS) and not rg_sleep:
                ...
                while True:
                    ...
                    if conf.RG:
                        ...
                        if not get_path("@app/sss.json").is_file():
                            raise Exception("未导入保全作业")
                        if not SSSNaviSolver().run():
